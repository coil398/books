...
## 後日談: 実行を明示的に促したら全コードが処理系を通った

この実験のあと、`sol@max` に `workspace-write` で修正させた。プロンプトには「可能なら自分の修正を検証してください」と書き、`tla2tools.jar` の場所を教えた。

結果、codex は **Lean 4.19.0 + Mathlib v4.19.0 を実際に導入して全コードブロックをコンパイル**し、以下を報告してきた。

| 検証 | 結果 |
|---|---|
| Lean コードブロック | 40 ブロックを個別検査、全成功。`-DwarningAsError=true` で失敗 0 |
| SANY | 全モジュール成功 |
| PlusCal Translator 1.12 | 2 例とも変換成功 |
| TLC | Counter 11 状態 / Inventory 3 状態 / Trade 4 状態 / DiscreteSum 6 状態 / Match 5 状態 |
| Vitest | 3 件成功 |
| Hypothesis | 7 件成功 |
| fast-check | 意図的失敗例で seed/path から反例 `[0,0,0,0,0]` を再現 |

さらに**指摘リストに無い問題を自分で見つけて直した**。

- 現行 `math` テンプレートの `autoImplicit = false` と教材内の暗黙型変数 `α` / `β` が矛盾していた
- `warningAsError` で落ちる不要な `simpa`
- 未使用引数の警告
- 実行例が実際には `main` を起動していなかった

> ⚠️ このうち **`autoImplicit` の件は、レビュー段階で人間側（Claude）が「false positive」として一度取り下げた指摘だった**。実際にコンパイルしたら本物だった。**推論による棄却が実測に負けた例**として記録しておく。

これは「ツールを実行させれば結果が変わる」という本レポートの結論を、修正フェーズでも再現したことになる。**レビューでも修正でも、LLM に処理系を触らせるかどうかが品質を決めている。**
